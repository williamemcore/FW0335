    ...
    if (0 == g_protect_counters[regs_to_protect])
    {
        ...
        /** When writing to the PRCR register the upper 8-bits must be the correct key. Set lower bits to 0 to
           ...
           b0     PRC0  - Enables writing to the registers related to the clock generation circuit:
                          SCKDIVCR, SCKSCR, MOSCCR, HOCOCR, MOCOCR, CKOCR, OSTDCR, OSTDSR, MOCOUTCR,
                          HOCOUTCR, MOSCWTCR, MOMCR, SOSCCR, SOMCR, LOCOCR, LOCOUTCR, HOCOWTCR.
           </PRE>
        */
...
void R_BSP_RegisterProtectDisable (bsp_reg_protect_t regs_to_unprotect)
{
    ...
    if (0 == g_protect_counters[regs_to_unprotect])
    {
        ...
        /** When writing to the PRCR register the upper 8-bits must be the correct key. Set lower bits to 1 to
           ...
           b15:b8 PRKEY - Write 0xA5 to upper byte to enable writing to lower byte
           b7:b4  Reserved (set to 0)
           b3     PRC3  - Enables writing to the registers related to the LVD: LVCMPCR, LVDLVLR,
           LVD1CR0, LVD1CR1, LVD1SR, LVD2CR0, LVD2CR1, LVD2SR.
           b2     Reserved (set to 0)
           b1     PRC1  - Enables writing to the registers related to operating modes, low power
           consumption, the clock generation circuit, and software reset: SYSCR1, SBYCR, MSTPCRA,
                          MSTPCRB, MSTPCRC, OPCCR, RSTCKCR, SOPCCR, MOFCR, MOSCWTCR, SWRR.
           b0     PRC0  - Enables writing to the registers related to the clock generation circuit:
           SCKCR, SCKCR3, PLLCR, PLLCR2, MOSCCR, SOSCCR,LOCOCR, ILOCOCR, HOCOCR, OSTDCR,
           OSTDSR, CKOCR.
           </PRE>
        */
...
/*******************************************************************************************************************//**
 * @brief Initialize variables needed for register protection functionality.
 **********************************************************************************************************************/
